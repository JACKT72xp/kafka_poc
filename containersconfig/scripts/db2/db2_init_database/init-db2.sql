...
FOR EACH ROW
BEGIN
    DECLARE v_id_estado VARCHAR(30);
    SET v_id_estado = CAST(NEW_ROW.ID AS VARCHAR(10)) || NEW_ROW.ESTADO;
    INSERT INTO PROCESO_BANCARIO_ALL_STATUS (ID_ESTADO, ID, ESTADO, NOMBRE, FECHA)
    VALUES (v_id_estado, NEW_ROW.ID, NEW_ROW.ESTADO, NEW_ROW.NOMBRE, NEW_ROW.FECHA);
END@
--#SET TERMINATOR ;
...
CREATE OR REPLACE VIEW VW_TRASPASOS 
AS
SELECT
    ID, ID_EXPEDIENTE, ID_FONDO_ORIGEN, ID_FONDO_DESTINO, FECHA, IMPORTE, CHGMARKER, ID AS CHGINC
FROM TRASPASOS;


CREATE OR REPLACE VIEW VW_RETENCIONES_1 
AS
SELECT
    ID, ID_ORDEN, FECHA_RETENCION, TIPO, IMPORTE_RETENCION, CHGMARKER, ID AS CHGINC
FROM retenciones_1;


CREATE OR REPLACE VIEW VW_RETENCIONES_2 
AS
SELECT
    ID, ID_TRASPASO, FECHA_RETENCION, TIPO, IMPORTE_RETENCION, CHGMARKER, ID AS CHGINC
FROM retenciones_2;
